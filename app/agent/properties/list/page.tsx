                    ...
                    {columns.map((col) => {
                      return (
                        <td key={col.key} className="px-4 py-2">
                          {col.key === "actions" ? (
                            editing && editing.id === property.id ? (
                              <div className="flex gap-2">
                                <button
                                  className="bg-blue-600 text-white px-3 py-1 rounded text-xs hover:bg-blue-700 border-2 border-blue-800"
                                  onClick={handleSaveEdit}
                                >
                                  Save
                                </button>
                                <button
                                  className="bg-gray-400 text-white px-3 py-1 rounded text-xs border-2 border-gray-600"
                          ...
                          ) : editing && editing.id === property.id && (col.key !== "listedAt" && col.key !== "updatedAt") ? (
                            col.key === "published" || col.key === "available" ? (
                              <select
                                className="border rounded px-2 py-1 text-xs"
                                value={String(editing.values[col.key])}
                                onChange={e => handleEditChange(col.key as keyof Property, e.target.value === "true")}
                              >
                                <option value="true">Yes</option>
                                <option value="false">No</option>
                              </select>
                            ) : col.key === "price" || col.key === "area" || col.key === "bedrooms" || col.key === "bathrooms" ? (
                              <input
                                type="number"
                              ...
                              />
                            ) : col.key === "title" || col.key === "slug" || col.key === "propertyType" ? (
                              <input
                                type="text"
